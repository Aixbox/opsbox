...
func (h *Handler) RegisterRoutes(group *gin.RouterGroup) {
	...
	g.POST("/connections/:id/scan", h.scan)
	// 控制台会话：本地模式会话即授权
...
// consoleWS 把控制台会话的事件流转给浏览器；客户端发来的 {"command":[...]} 直接在面板里执行。
func (h *Handler) consoleWS(c *gin.Context) {
	...
	defer conn.Close()
	// replay=true：新接入 / 断线重连的panel先补回历史事件，不至于空屏
	...
	// 面板里手输的命令走这条路径执行（来源标记为 web，与其他 CLI 操作区分展示）。
	// 用脱离请求的上下文：WebSocket 是长连接，把命令执行挂在请求上下文上，
	// 请求一旦超时/取消，后续每条命令都会在 GetConnection 处就失败——面板将永远收不到回显。
	...
	for {
		...
		go func(args []string) {
			...
			execCtx, cancel := context.WithTimeout(context.WithoutCancel(c.Request.Context()), 60*time.Second)
			defer cancel()
			_, _ = h.service.Exec(execCtx, session.UserID, session.ConnectionID, ExecRequest{Args: args, Source: "web"})
		}(message.Args)
	}
...
func fail(c *gin.Context, err error) {
	...
	case errors.Is(err, ErrInvalid):
		status, code, message = http.StatusUnprocessableEntity, "INVALID_REDIS_REQUEST", err.Error()
	...
	case errors.As(err, new(*opscheck.RequiredError)):
		// 需审批写命令缺少预检令牌：409 + 完整预检结果（含令牌），CLI 据此走「先确认再重提」协议
		var required *opscheck.RequiredError
		_ = errors.As(err, &required)
		response.JSON(c, http.StatusConflict, "REDIS_CHECK_REQUIRED", required.Check.Guidance, required.Check)
		return
	default:
		// 拨号 / 认证失败 / Redis 错误（WRONGTYPE、MOVED 等）：把原因带给调用方（CLI 需要看到）
		if strings.Contains(err.Error(), "连接") || strings.Contains(err.Error(), "redis:") || strings.Contains(err.Error(), "Redis") || strings.Contains(err.Error(), "MOVED") || strings.Contains(err.Error(), "dial") {
			status, code, message = http.StatusBadGateway, "REDIS_UPSTREAM_ERROR", err.Error()
		}
	}
	response.Error(c, status, code, message, nil)
}

// extendDeadline 放宽本次请求的网络死线（服务器默认 30s 写超时会打断慢命令与全量预热）。
...
func (h *Handler) createConnection(c *gin.Context) {
	...
	response.Created(c, "/api/v1/redis/connections", item.ID, item)
}
...
func (h *Handler) exec(c *gin.Context) {
	...
	extendDeadline(c, 45*time.Second)
...
func (h *Handler) openSession(c *gin.Context) {
	...
	response.Created(c, "/api/v1/redis/connections/"+strconv.FormatInt(id, 10)+"/sessions", 0, info)
}
...
func (h *Handler) scan(c *gin.Context) {
	...
	extendDeadline(c, 60*time.Second)
	result, err := h.service.Scan(c.Request.Context(), currentUser(c), id, input)
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) listLogs(c *gin.Context) {
	filter := LogFilter{Status: c.Query("status")}
	...
	for _, item := range []struct {
		...
		if raw := c.Query(item.name); raw != "" {
			...
			if err != nil {
				response.Error(c, http.StatusBadRequest, "INVALID_REQUEST", item.name+" 必须是 RFC3339 时间", nil)
				return
			}
			*item.target = &parsed
		}
