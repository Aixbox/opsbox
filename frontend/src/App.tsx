...
// 模块内二级视图：连接管理 / 执行日志。
// .tabs--sub 为 styles.css 里的紧凑款标签（比主导航小一号、不撑满整行），
// 明确「连接/日志」是模块内二级切换，与顶部 SSH/数据库/Redis 主导航拉开层级。
function ModulePanel({ label, main, logs }: { label: string; main: ReactNode; logs: ReactNode }) {
  const [view, setView] = useState("main");
  return (
    <Tabs selectedKey={view} onSelectionChange={(key) => setView(String(key))} className="tabs--sub">
      <Tabs.ListContainer>
